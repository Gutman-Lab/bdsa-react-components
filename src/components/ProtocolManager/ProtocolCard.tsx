...
export function ProtocolCard({
    ...
    const isBlock = protocol.type === 'block';
    const regionById = new Map(regionProtocols.map((p) => [p.id, p]));
    const isIgnore =
        protocol.id === 'ignore' ||
        ...
        <div className="protocol-card">
            ...
            <div className="protocol-details">
                ...
                {isBlock ? (
                    <>
                        <p>
                            <strong>Mappings:</strong> {protocol.slots?.length ?? 0} block
                            {(protocol.slots?.length ?? 0) === 1 ? '' : 's'}
                        </p>
                        {protocol.slots && protocol.slots.length > 0 && (
                            <ul className="block-slot-summary">
                                {protocol.slots.map((slot) => {
                                    const rp = regionById.get(slot.regionProtocolId);
                                    return (
                                        <li key={`${slot.blockId}-${slot.regionProtocolId}`}>
                                            <code>{slot.blockId}</code>
                                            {' → '}
                                            {rp?.name ?? slot.regionProtocolId}
                                        </li>
                                    );
                                })}
                            </ul>
                    ...
                    <>
                        ...
                        {protocol.chemistry && (
                            <p>
                                <strong>Chemistry:</strong> {protocol.chemistry}
                            </p>
                        )}
                        {protocol.chromogen && (
                            <p>
