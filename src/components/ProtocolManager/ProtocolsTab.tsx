...
import { BDSA_SCHEMA_DEFAULT_OPTIONS } from './utils/schemaValidator';
...
export function ProtocolsTab({
    ...
    useBundledBdsaSchema = true,
    title = 'Protocols',
    description = 'Manage stain, region, and block protocols for BDSA schema compliance',
    ...
    const [activeSubTab, setActiveSubTab] = useState<'stain' | 'region' | 'block'>('stain');
    ...
    const noOpFallbackRef = useRef(new NoOpDsaSyncAdapter())
    const adapter = useMemo(
        (): DsaSyncAdapter => dsaSyncAdapter ?? noOpFallbackRef.current,
        [dsaSyncAdapter],
    )
    ...
    useEffect(() => {
        const loadSchemas = async () => {
            if (!schemaValidator.loaded) {
                if (useBundledBdsaSchema) {
                    await schemaValidator.loadSchemas(undefined, undefined, BDSA_SCHEMA_DEFAULT_OPTIONS);
                } else {
        ...
        loadSchemas();
    }, [schemaValidator, useBundledBdsaSchema]);
    ...
    useEffect(() => {
        if (onProtocolChange) {
            onProtocolChange({
                ...
                regionProtocols,
                blockProtocols,
                total: stainProtocols.length + regionProtocols.length + blockProtocols.length,
            });
        }
    }, [stainProtocols, regionProtocols, blockProtocols, onProtocolChange]);

    const handleAddProtocol = () => {
        const regionCount = regionProtocols.filter(
            (p) => !p._isDefault && p.regionType !== 'ignore',
        ).length;
        if (activeSubTab === 'block' && regionCount === 0) {
            alert('Create at least one region protocol before adding a block protocol.');
            return;
        }
    ...
    return (
        <div className="protocols-tab">
            ...
            <div className="protocols-navigation">
                ...
                <button
                    className={`nav-button ${activeSubTab === 'block' ? 'active' : ''}`}
                    onClick={() => setActiveSubTab('block')}
                >
                    Block Protocols ({blockProtocols.length})
                </button>
            </div>
