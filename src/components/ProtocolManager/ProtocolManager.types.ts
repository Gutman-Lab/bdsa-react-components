...
export type ProtocolType = 'stain' | 'region' | 'block';

/** One block ID mapped to a region protocol in this collection. */
...
export interface Protocol {
    ...
    /** Block protocol: reusable block ID → region protocol template */
...
export interface DsaSyncAdapter {
    ...
    pullProtocols(): Promise<{
        success: boolean;
        pulled?: { stainProtocols: number; regionProtocols: number; blockProtocols?: number };
        error?: string;
    }>;
    pushProtocols(
        ...
        success: boolean;
        pushed?: { stainProtocols: number; regionProtocols: number; blockProtocols?: number };
        error?: string;
    }>;
...
export interface SchemaValidator {
    ...
    loadSchemas(
        schemaUrl?: string,
        schemaData?: unknown,
        options?: { useDefault?: boolean; stainSchemaUrl?: string; regionSchemaUrl?: string },
    ): Promise<boolean>;
...
export interface ProtocolsTabProps {
    ...
    schemaValidator: SchemaValidator; // Required - must be provided to load options from BDSA schema
    /**
     * When true (default), loads Pitt stain + region schemas bundled with npm (`useDefault`).
     */
    useBundledBdsaSchema?: boolean;
    title?: string;
    description?: string;
