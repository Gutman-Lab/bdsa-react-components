/* eslint-disable react-refresh/only-export-components -- module exports shared opacity helpers for OSDThumbnailBrowser */
...
export interface ThumbnailViewerProps {
    ...
    viewerWidth: number
    /** DSA API base URL including `/api/v1`. When omitted, uses `dsaAuthStore`. */
    apiBaseUrl?: string
    ...
    /** Overrides Girder token from the store when set. */
    authToken?: string
    /** Passed through to SlideViewer for tile/DZI URLs. */
...
export const ThumbnailViewer = React.memo<ThumbnailViewerProps>(
    ({
        ...
        const imageInfo = useMemo(() => {
            return {
                dziUrl: `${effectiveApiBaseUrl}/item/${item._id}/tiles/dzi.dzi`,
            }
        }, [item._id, effectiveApiBaseUrl])
        ...
        const proxyFetchFn = useCallback(
            ...
            [effectiveApiBaseUrl, backendApiBaseUrl, effectiveApiHeaders, fetchFn, annotationId, debug, debugLog]
        )
        ...
        useEffect(() => {
            ...
            const syncCanvas = () => {
                ...
                if (osdContainer && paperCanvas) {
                    ...
                    const canvasEl = paperCanvas as HTMLElement
        ...
        const annotationOpacities = useMemo(() => {
            ...
            // opacityMapVersion bumps when shared map changes without annotationId changing
        ...
        useEffect(() => {
            ...
            const checkForPaperItems = () => {
                ...
                if (canvas) {
                    try {
                        type PaperScopeLite = {
                            project?: { activeLayer?: { children?: { length: number } }; canvas?: HTMLCanvasElement }
                        }
                        const w = window as Window & { paper?: { scopes?: PaperScopeLite[] } }
                        const paperScope = w.paper?.scopes?.find((scope) => scope.project?.canvas === canvas)
                        const n = paperScope?.project?.activeLayer?.children?.length ?? 0
                        if (n > 0) {
                            debugLog.log(`[ThumbnailViewer] Detected ${n} Paper.js items, annotation is rendering`)
                            return true
                        }
                    } catch (e) {
    ...
    (prevProps, nextProps) => {
        ...
        return (
            ...
            prevProps.apiHeaders === nextProps.apiHeaders &&
            prevProps.authToken === nextProps.authToken &&
            prevProps.tokenQueryParam === nextProps.tokenQueryParam
        )
    }
)
