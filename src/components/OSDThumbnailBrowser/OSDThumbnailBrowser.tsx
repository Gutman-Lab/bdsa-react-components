...
import { useDsaResolvedApi, headersInitToRecord } from '../../auth/useDsaResolvedApi'
...
import './OSDThumbnailBrowser.css'

export interface OSDThumbnailBrowserProps {
     ...
     * When omitted, uses `dsaAuthStore` (same as FolderBrowser and ThumbnailGrid).
     */
    apiBaseUrl?: string
    ...
    authToken?: string
    /** Append token to tile/DZI requests (passed through to SlideViewer). */
 ...
 */
export const OSDThumbnailBrowser = React.forwardRef<HTMLDivElement, OSDThumbnailBrowserProps>(
    (
        {
            folderId,
            imageIds,
        ...
        const assignContainerRef = useCallback(
            (node: HTMLDivElement | null) => {
                ...
                if (typeof ref === 'function') {
                    ...
                    const forwarded = ref as { current: HTMLDivElement | null }
        ...
        const debugLog = useMemo(() => createDebugLogger('OSDThumbnailBrowser', debug), [debug])
        ...
        const proxyFetchFn = useCallback(
            async (url: string, options?: RequestInit): Promise<Response> => {
                ...
                if (annotationSearchMatch || annotationIdMatch) {
                    const backendUrl = `${backendApiBaseUrl}/api/v1${url.replace(effectiveApiBaseUrl, '').replace(/^\/api\/v1/, '')}`
                    const backendOptions: RequestInit = {
                        ...options,
                        headers: {
                            ...(effectiveApiHeaders ? headersInitToRecord(effectiveApiHeaders) : {}),
            ...
            [effectiveApiBaseUrl, backendApiBaseUrl, effectiveApiHeaders, fetchFn]
        )
        ...
        const fetchItems = useCallback(async () => {
            ...
            try {
                ...
                if (folderId) {
                    ...
                    const url = `${effectiveApiBaseUrl}/item?${params.toString()}`
                    debugLog.log(`Fetching items from: ${url}`)
                    
                    const fetchOptions: RequestInit = {}
                    if (effectiveApiHeaders) {
                        fetchOptions.headers = effectiveApiHeaders
                    }
                    ...
                    for (const itemId of imageIds) {
                        const url = `${effectiveApiBaseUrl}/item/${itemId}`
                ...
                const url = folderId 
                    ? `${effectiveApiBaseUrl}/item?folderId=${folderId}`
                    : `${effectiveApiBaseUrl}/item`
            ...
            // eslint-disable-next-line react-hooks/exhaustive-deps -- processItems is defined below; fetchFn covered via proxyFetchFn when used
        }, [folderId, imageIds, providedItems, backendApiBaseUrl, effectiveApiBaseUrl, effectiveApiHeaders, proxyFetchFn, debugLog, onApiError])
        ...
        const renderPagination = () => {
            ...
            return (
                <div className="bdsa-osd-thumbnail-browser__pagination">
                    <button
                        className="bdsa-osd-thumbnail-browser__pagination-button"
                    ...
                    {pages.map((page, idx) => {
                        if (page === 'ellipsis') {
                            return (
                                <span key={`ellipsis-${idx}`} className="bdsa-osd-thumbnail-browser__pagination-ellipsis">
                                    ...
                                </span>
                            )
                        }
                        return (
                            <button
                                key={page}
                                className={`bdsa-osd-thumbnail-browser__pagination-button ${
                                    page === currentPage ? 'bdsa-osd-thumbnail-browser__pagination-button--active' : ''
                                }`}
        ...
        return (
            <div ref={assignContainerRef} className={`bdsa-osd-thumbnail-browser ${className}`}>
                <div className="bdsa-osd-thumbnail-browser__header">
                    <div className="bdsa-osd-thumbnail-browser__header-left">
                        <h3 className="bdsa-osd-thumbnail-browser__title">
                            {modelName || 'Thumbnail Browser'}
                        </h3>
                        <span className="bdsa-osd-thumbnail-browser__count">
                            {filteredItems.length} item{filteredItems.length !== 1 ? 's' : ''}
                        </span>
                    </div>
                    <div className="bdsa-osd-thumbnail-browser__header-right">
                        <div className="bdsa-osd-thumbnail-browser__size-controls">
                            {(['s', 'm', 'l', 'xl'] as const).map((size) => (
                                <button
                                    key={size}
                                    className={`bdsa-osd-thumbnail-browser__size-button ${
                                        viewerSize === size ? 'bdsa-osd-thumbnail-browser__size-button--active' : ''
                                    }`}
                        ...
                        {selectedAnnotationName && (
                            <div className="bdsa-osd-thumbnail-browser__opacity-control">
                                <label className="bdsa-osd-thumbnail-browser__opacity-label">
                                ...
                                <input
                                    ...
                                    className="bdsa-osd-thumbnail-browser__opacity-slider"
                                />
                            </div>
                        )}
                    </div>
                ...
                {renderPagination()}
                {loading && (
                    <div className="bdsa-osd-thumbnail-browser__loading">
                        <p>Loading items...</p>
                    </div>
                )}
                {error && (
                    <div className="bdsa-osd-thumbnail-browser__error">
                        <p>Error: {error.message}</p>
                    </div>
                )}
                {!loading && !error && (
                    <div ref={gridRef} className="bdsa-osd-thumbnail-browser__grid">
                ...
                {!loading && !error && filteredItems.length === 0 && (
                    <div className="bdsa-osd-thumbnail-browser__empty">
...
OSDThumbnailBrowser.displayName = 'OSDThumbnailBrowser'
