 ...
 * ```
 */
export function filterLargeImages(items: Item[]): Item[] {
 ...
 * ```
 */
export function isAIModel(item: Item): boolean {
    const meta = item.meta || {}
    // An item is considered an AI model if it has both dataset_args and train_args
 ...
 * ```
 */
export function filterAIModels(items: Item[]): Item[] {
